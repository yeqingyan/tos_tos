 ...
 */
WORD get_fore_colour() { return foreground_color; }
 ...
 */
WORD get_back_colour() { return background_color; }
...
void set_pixel(int x, int y) {
  ...
  height = graphicsAddress->p_height;
  height--;
  if (y > height)
    goto ERROR;

  width = graphicsAddress->p_width;
  width--;
  if (x > width)
    goto ERROR;
...
void clear_pixel(int x, int y) {
  ...
  width++;
  // Calculate pixel position in memory
  *(gpu_pointer + (x + y * width)) = background_color;
ERROR:
  return;
}
...
WORD get_pixel_16bit(int x, int y) {
  ...
  if (y > (graphicsAddress->p_height - 1))
    goto ERROR;
  if (x > (graphicsAddress->p_width - 1))
    goto ERROR;
...
void copy_pixel_16bit(int src_x, int src_y, int des_x, int des_y) {
  if (src_y > (graphicsAddress->p_height - 1))
    goto ERROR;
  if (des_y > (graphicsAddress->p_height - 1))
    goto ERROR;
  if (src_x > (graphicsAddress->p_width - 1))
    goto ERROR;
  if (des_x > (graphicsAddress->p_width - 1))
    goto ERROR;
  WORD *gpu_pointer;

  gpu_pointer = (WORD *)graphicsAddress->gpu_pointer;

  *(gpu_pointer + (des_x + des_y * graphicsAddress->p_width)) =
      *(gpu_pointer + (src_x + src_y * graphicsAddress->p_width));
  return;
ERROR:
  return;
}
