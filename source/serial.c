...
void serial_reader_process(PROCESS self, PARAM param) {
  ...
  while (1) {
    index = 0;
    // receive message form Serial process, this message contains the number of
    // bytes to read in Serial_Message.len_input_buffer
...
void serial_process(PROCESS self, PARAM param) {
  ...
  serial_reader_port =
      create_process(serial_reader_process, 7, 0, "Serial Reader Process");

  while (1) {
    // receive message from user process
    Serial_Message *msg = (Serial_Message *)receive(&sender);
    // We can send string here, but since train emulation return byte by byte,
    // we send string
    // byte by byte.
    ...
    // wait for message from COM reader process that signals all bytes have been
    // read
