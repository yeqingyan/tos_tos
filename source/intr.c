...
/*
    ...
    TODO: Change Timer IRQ from basic irq to IRQs_1. change for loop to link
   list.
      */
/**
...
void irq_handler(void) {
  ...
  //            if ((((GetIRQController()->IRQ_Basic_Pending) & (1 <<
  //            current_irq)) > 0) &&
  ...
  //    if (enable_usb && ((GetIRQController()->IRQ_Pending_1) & (1 << USB_IRQ))
  //    > 0) {
 ...
 * Ref. BCM2835 1.3
 * Ref. Xinu project
 * https://github.com/xinu-os/xinu/blob/4384915cc63a9c057418015c996f44372eec0a2e/docs/arm/rpi/BCM2835-Memory-Barriers.rst
 * Ref. Xinu memory barreir function
 * https://github.com/xinu-os/xinu/blob/4384915cc63a9c057418015c996f44372eec0a2e/system/arch/arm/memory_barrier.S
 */
void dmb(void) {
  asm("mov r12, #0");
...
void master_isr(void) {
  // When CPU into IRQ mode, the IRQ Link Register(R14_IRQ) will have value 
  // PC+4, where PC is the address of the instruction that was NOT executed 
  // because the IRQ took priority. In order to return to the right address(PC),
