...
void led_proc(PROCESS self, PARAM param) {
  ...
  // GPIO Pull-up/down register: Set bit 1-0 to 10 to enable default input to
  // HIGH
  ((GPPUD_REG *)(gpio_base_addr + GPPUD_OFFSET))->pud = 2;
  wait_150_cycles(); // According BCM2835 manual, wait 150 cycles before next
                     // step.
  ...
  while (1) {
    SAVE_CPSR_DIS_IRQ(cpsr_flag);
    if (((GPLEV_REG *)(gpio_base_addr + GPLEV_OFFSET))->pin8 ==
        0) { // Check Pin 8's level
      if (led_status == 0) {
        wprintf(led_ptr, "Turn on LED\n");
        led_status = 1;
        // need to use set_gpio() otherwise USB do not work, maybe have
        // something to do with Memory Barrier mentioned in BCM2835 manual. TODO
        // set_gpio(PIN_OUT, 1);
        ((GPSET_GPCLR_REG *)(gpio_base_addr + SET_OFFSET))->pin7 =
            1; // If Pin 8 is high, turn Pin 7 on to light on LED
      }
    } else {
      if (led_status == 1) {
        ...
        // something to do with Memory Barrier mentioned in BCM2835 manual. TODO
        //                set_gpio(PIN_OUT, 0);
        // Very strange problem, if we use the following instruction, USB will
        // failed on transfer packet..
        dmb();
        ((GPSET_GPCLR_REG *)(gpio_base_addr + CLR_OFFSET))->pin7 =
            1; // If Pin 8 is low, turn Pin 7 off to light off LED
        dmb();
      }
    }
    RESUME_CPSR(cpsr_flag);
