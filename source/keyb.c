...
BYTE new_key;
PORT keyb_port;
BYTE keys_normal[104] = {
    0x0,  0x0,  0x0, 0x0,  'a',  'b', 'c', 'd', 'e', 'f',  'g',  'h', 'i',
 ...
 * return: True if key already been press down in previous loop, otherwise
 * return False.
 */
...
void reset_usb_transfer_result() {
  //    volatile unsigned int cpsr_flag;
  //    SAVE_CPSR_DIS_IRQ(cpsr_flag); // Makesure isr do not change
  //    usb_transfer_result at the same time.
...
void usbkeyb_process(PROCESS self, PARAM param) {
  ...
  while (1) {
    ...
    //        while(1);
    //        while ((usb_transfer_result == USB_TRANSFER_NULL) ||
